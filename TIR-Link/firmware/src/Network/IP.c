...
TIR_Status ipv4StringToAddr(const char *str, Ipv4Addr *ipAddr)
{
    TIR_Status status;
    int16_t i = 0;
    int16_t value = -1;
...
bool isFragmentedPacket(Ipv4Header *packet) {
    return (be16toh(packet->fragmentOffset) & (IPV4_FLAG_MF | IPV4_OFFSET_MASK)) != 0;
}
...
void ipv4DumpHeader(const Ipv4Header *ipHeader)
{
    ...
    printDebug("## IPv4 Section\r\n");
    printDebug("  Version = %" PRIu8 "\r\n", ipHeader->version);
    printDebug("  Header Length = %" PRIu8 "\r\n", ipHeader->headerLength);
    printDebug("  Type Of Service = %" PRIu8 "\r\n", ipHeader->typeOfService);
    printDebug("  Total Length = %" PRIu16 "\r\n", betoh16(ipHeader->totalLength));
    printDebug("  Identification = %" PRIu16 "\r\n", betoh16(ipHeader->identification));
    printDebug("  Flags = 0x%01X\r\n", betoh16(ipHeader->fragmentOffset) >> 13);
    printDebug("  Fragment Offset = %" PRIu16 "\r\n", betoh16(ipHeader->fragmentOffset) & 0x1FFF);
    printDebug("  Time To Live = %" PRIu8 "\r\n", ipHeader->timeToLive);
    printDebug("  Protocol = %" PRIu8 "\r\n", ipHeader->protocol);
    printDebug("  Header Checksum = 0x%04" PRIX16 "\r\n", betoh16(ipHeader->headerChecksum));
